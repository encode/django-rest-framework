    ...
    * Default `.create()` and `.update()` implementations are provided.
    ...
    def build_standard_field(self, field_name, model_field):
        ...
        if 'choices' in field_kwargs:
            ...
            # And also check if `MultipleChoiceField` is needed.
            field_defined_as_multiple = getattr(model_field, 'serializer_multiple_choices', None)
            is_multi_choice = bool(field_defined_as_multiple)
            if field_defined_as_multiple is None:
                # Check formfield if this field uses multiple choices as default.
                try:
