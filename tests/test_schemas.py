...
@unittest.skipUnless(coreapi, 'coreapi is not installed')
def setUp(self):
    self.patterns = [    url(r'^example/?$', ExampleListView.as_view()),    url(r'^example/(?P<pk>\d+)/?$', ExampleDetailView.as_view()),    url(r'^example/(?P<pk>\d+)/sub/?$', ExampleDetailView.as_view()),    ]
...
def setUp(self):
    self.patterns = [    path('example/', ExampleListView.as_view()),    path('example/<int:pk>/', ExampleDetailView.as_view()),    path('example/<int:pk>/sub/', ExampleDetailView.as_view()),    ]
...
@unittest.skipUnless(coreapi, 'coreapi is not installed')
def setUp(self):
    self.patterns = [    url(r'^api/v1/example/?$', ExampleListView.as_view()),    url(r'^api/v1/example/(?P<pk>\d+)/?$', ExampleDetailView.as_view()),    url(r'^api/v1/example/(?P<pk>\d+)/sub/?$', ExampleDetailView.as_view()),    ]
...
deftest_manually_routing_nested_routes(self):
    ...
    patterns = [    url(r'example/(?P<pk>\d+)/$', BasicNamingCollisionView.as_view()),    url(r'example/(?P<slug>\w+)/$', BasicNamingCollisionView.as_view()),    ]
    generator = SchemaGenerator(title='Naming Colisions', patterns=patterns)
    schema = generator.get_schema()
    ...
    patterns = [    url(r'^test/list/', simple_fbv),    url(r'^test/(?P<pk>\d+)/list/', simple_fbv),    ]
    generator = SchemaGenerator(title='Naming Colisions', patterns=patterns)
    schema = generator.get_schema()
